...
public class FileUtil {
	...
	/*
	 ...
	 따라서 혹시 같은 이름의 파일이 존재하면 파일 이름을 바꿔서 저장 해줘야 한다.
	 이전에 업로드했던 동일한 이름의 파일도 유지가 되고
	 내가 현재 업로드하는 파일도 저장이 EHLSEK.
...
public static String rename(String path, String oldName) {
	...
	/*
		 정책 설정]
		 	...
		 	혹시 같은 이름의 파일이 존재하면 _1을 붙여서 이름일 바꾸는 방식을 사용하겠다.
		 	
		 	예]
		 		sample.jpg 라는 파일을 업로드 하려하는데
	`			이미 업로드 된 파일의 이름이 존재한다면 
		 		sample_1.jpg로 바꿔서 사용한다.
 ...
 */
	...
	String tmpName = oldName; //현재이름을 따로 기억해 놓는다.
	 ...
	 while(file.exists()) {
		 ...
		 //이미 파일이 존재하는 경우는 
		 // 이름을 바꿔줘야 한다.
		 // 이때 우리는_에 숫자를 붙여서 파일이름을 만들기로 했다.
		count++;
